...
async def seed_on_startup():
    ...
    try:
        ...
        if existing == 0:
            logging.info("🌱 Database is empty. Auto-seeding products and kits...")
            from seed_hybrid_data import HYBRID_PRODUCTS, BUY_ONLY_PRODUCTS, CURATED_KITS
            import copy

            for product in HYBRID_PRODUCTS:
                await db.products.insert_one(copy.deepcopy(product))
            logging.info(f"  ✓ Seeded {len(HYBRID_PRODUCTS)} hybrid products")

            for product in BUY_ONLY_PRODUCTS:
                await db.products.insert_one(copy.deepcopy(product))
            logging.info(f"  ✓ Seeded {len(BUY_ONLY_PRODUCTS)} buy-only products")

            for kit in CURATED_KITS:
                await db.kits.insert_one(copy.deepcopy(kit))
            logging.info(f"  ✓ Seeded {len(CURATED_KITS)} curated kits")

            logging.info("✅ Database seeded successfully!")
        else:
            logging.info(f"📦 Database already has {existing} products. Skipping seed.")
    except Exception as e:
        logging.warning(f"⚠️ Could not seed database: {e}")
